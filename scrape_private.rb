# Scrape linkedin profile from saved html of private profile

# Purpose:  Almost half of profiles are (fully or partially) hidden on public-facing web. To collect the full profile, log into linkedin account and go to profile page e.g.https://www.linkedin.com/in/anandrajaraman and save page as html file

# Here are the main-steps in the process:
# Step 1: Run the script and install any missing gems
# Step 2: Obtain profile data by linking the file location:
#    path = "/Users/tarun/Ruby200/Alerts/Alerts_02_21_2016/Anand Rajaraman _ LinkedIn.html"
#    profile = Linkedin::Profile.get_profile("file:" + path)
# Step 3: Convert ruby object to json (profile.to_json or Yajl.dump(profile))
...
require "net/http"
  ...
  module Linkedin
    ...
    def initialize(url)
        ...
        person_link = @page.at('.view-public-profile')
        if person_link
            final = @page.at('.view-public-profile').text
        else
            final = "https://www.linkedin.com/vsearch/p?keywords="+ @page.at('.full-name').text.gsub(' ','+')
        end
        @linkedin_url = final
    end
    ...
    def get_companies(type)
        ...
        if @page.search(".background-experience .#{type}-position").first
            @page.search(".background-experience .#{type}-position").each do |node|
                ...
                result = { :linkedin_company_url => "linkedin.com" + company_link }
                companies << company.merge!(result) #companies << company
            end
        end
        companies
    end
