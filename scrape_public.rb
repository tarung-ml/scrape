# Scrape public linkedin profile using public profile URL

# Here are the main-steps in the process:
# Step 1: Run the script and install any missing gems
# Step 2: Obtain profile object as below:
#    profile = Linkedin::Profile.get_profile("https://www.linkedin.com/in/anandrajaraman")
# Step 3: Convert object to json (profile.to_json or Yajl.dump(profile))
...
module Linkedin
    ...
    private
    #store companies in an array of hashes
    def get_companies()
        if @companies
            return @companies
            else
        ...
        @page.search(".positions .position").each do |node|
            ...
            if company_link
                result = get_company_details(company_link) #disable if company information not required
